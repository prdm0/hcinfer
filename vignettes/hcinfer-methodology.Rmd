...
Small samples can make moment estimates unstable. HCbeta therefore shrinks the
estimated shape parameters toward the uniform case $a = b = 1$ and then clamps
the resulting values. The regularized shape parameters are

$$
\zeta = \frac{n}{n + 50},
\qquad
\tilde a =
\min\{\max\{(1 - \zeta) + \zeta \hat a,\, \varepsilon\},\, A_{\max}\},
\qquad
\tilde b =
\min\{\max\{(1 - \zeta) + \zeta \hat b,\, \varepsilon\},\, B_{\max}\},
\qquad
\varepsilon = 0.01.
$$

For smaller $n$, $\zeta$ is smaller and the method stays closer to the uniform
baseline, whereas the estimated shape parameters receive more weight as $n$
grows. The fixed floor $\varepsilon = 0.01$ is applied after shrinkage and
before the upper caps, which keeps both adjusted shapes strictly positive.
Unlike `lower`, which truncates $w_t$, the floor is not a method argument and
does not change when the truncation interval is varied.

The caps $A_{\max} = B_{\max} = 10000$ correspond to `a_max` and `b_max`, each
with the inclusive range `[50, 25000]`. They keep the shape parameters bounded
and can be changed independently for sensitivity analysis. Together with the
fixed floor, the caps place $\tilde a$ and $\tilde b$ in a compact interval.
Because $w_t$ is also truncated, $\log F_B(w_t; \tilde a, \tilde b)$ remains
uniformly bounded, so the exponent $c_1/n^{c_2}$ governs the asymptotic
behavior and $g_t \to 1$ regardless of the caps.

When every truncated complement is identical, $s_w^2 = 0$ and the raw
method-of-moments shape estimates diverge. In that degenerate low-leverage
regime, `hcinfer` sets $\tilde a = A_{\max}$ and
$\tilde b = B_{\max}$ rather than aborting. The Beta cdf at the common upper
truncation is then numerically one under the default caps, so the HCbeta factor
reduces to the HC1 scale $n/(n - p)$ and remains well-defined as it converges to
one.
...
HCbeta uses `stats::pbeta()` for the Beta cdf, and the incomplete beta function
is not reimplemented inside the package. The cdf is evaluated on the log scale
with `pbeta(w, a_tilde, b_tilde, log.p = TRUE)`, and the resulting exponent is
capped at 700 before exponentiation. This prevents both
$\log(0) = -\infty$ when the cdf underflows and floating-point overflow in
`exp()` for extreme shape
parameters. The degrees of freedom factor $n/(n - p)$ is computed once.

Method-specific constants are passed through `...` in `vcov_hc()` and
`hcinfer()`. The HCbeta caps can be supplied independently through `...`, and
unknown names are rejected. This keeps the interface compact while making the
