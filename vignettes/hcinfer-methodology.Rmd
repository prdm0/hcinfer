...
`b_max` (minimum 50). They keep the shape parameters bounded and act only as a
numerical safeguard. Because the truncated $w_t$ together with bounded
$\tilde a$ and $\tilde b$ keep $\log F_B(w_t; \tilde a, \tilde b)$ uniformly
...
capped before exponentiation. This prevents both $\log(0) = -\infty$ when the
cdf underflows and floating point overflow in `exp()` for extreme shape
