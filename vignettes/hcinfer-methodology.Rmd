...
\zeta = \frac{n}{n + 50},
\qquad
\tilde a = \min\{(1 - \zeta) + \zeta \hat a,\, A_{\max}\},
\qquad
\tilde b = \min\{(1 - \zeta) + \zeta \hat b,\, B_{\max}\}.
$$
...
The caps $A_{\max} = B_{\max} = 10000$ correspond to the arguments `a_max` and
`b_max`, each with the inclusive range `[50, 25000]`. They keep the shape
parameters bounded and act only as a numerical safeguard. Because the truncated
$w_t$ together with bounded $\tilde a$ and $\tilde b$ keep
$\log F_B(w_t; \tilde a, \tilde b)$ uniformly
bounded, the exponent $c_1/n^{c_2}$ alone governs the asymptotic behavior, and
$g_t \to 1$ regardless of the cap.
