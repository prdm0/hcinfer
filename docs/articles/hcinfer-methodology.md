...
the HC5 adjustment follows the corrected expression given in the erratum
to Cribari-Neto, Souza and Vasconcellos (2007),

g_t = (1 - h_t)^{-\delta_t / 2}, \qquad \delta_t = \min\left(
\frac{h_t}{\bar h}, \max\left(4, k \frac{h\_{max}}{\bar h}\right)
\right), \qquad k = 0.7.

The HC5m adjustment combines the HC4m structure with the HC5 term and
applies the resulting exponent without the factor 1/2:
